...
// 文件批量打印工具
// 作者: 王国强  Rev.A01
// 版本: A0 (完整版，Email位于文件列表标题行，优化打印隐藏)
// 搜索关系：或（任意关键词匹配即可）
    ...
    public class Form1 : Form
    {
        ...
        private void PrintTextFile(string filePath)
        {
            ...
            pd.PrintPage += (sender, e) =>
            {
                ...
                {
                    line = lines[count];
                    yPos = topMargin + (count * pd.DefaultPageSettings.PrinterResolution.Y / 100 * 0.3f); // 行高约12磅
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in clbFiles.CheckedItems)
            {
                ...
                try
                {
                    ...
                    // 对于文本文件，使用PrintDocument直接打印（完全静默）
                    if (ext == ".txt")
                    {
                        PrintTextFile(file);
                    }
                    ...
                    success++;
                    System.Threading.Thread.Sleep(1500); // 等待打印任务提交
                }
                catch (Exception ex)
                {
                    fail++;
